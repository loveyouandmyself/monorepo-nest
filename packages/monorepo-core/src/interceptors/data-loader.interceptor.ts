...
@Injectable()
export class MyDataLoaderInterceptor implements NestInterceptor {
...
export const Loader = createParamDecorator(async (data: any, context: ExecutionContext & { [key: string]: any }) => {
  ...
  if (ctx[NEST_LOADER_CONTEXT_KEY] === undefined) {
    throw new InternalServerErrorException(`
            You should provide interceptor ${MyDataLoaderInterceptor.name} globally with ${APP_INTERCEPTOR}
          `);
  }
  const result = await ctx[NEST_LOADER_CONTEXT_KEY].getLoader(data);
